...
class PainvoiceController extends AdminController {
    /**
     * The invoice id
     * @var int
     */
    ...
    public function getStatusinvoices($statusId)
    {
        ...
        if(count($this->agreements))
        {
            $invoices =  Mainvoice::join('mainvoicestatus','mainvoicestatus.mainvoice_id','=','mainvoice.id')
                ...
                ->select('mainvoice.id','mainvoice.description','merchantagreement.name','merchant.merchant','mainvoice.date_from','mainvoice.date_to','mainvoice.created_at','mainvoice.processcurrency','mainvoice.payoutcurrency', 'mainvoice.amount')
                ->get();
    ...
    public function getShow($invoiceId)
    {
        ...
        if($invoice)
        {
            ...
            return View::make('backend/mainvoice/show', compact('invoice','specificationrows', 'costrows', 'reportrows', 'holdbackrows', 'customrows', 'heldrows', 'incomerows','merchantagreement','conversionrate', 'payments', 'haspayments'));
        }
        else{
            return Redirect::to("admin/mainvoice/")->with('error', 'Access denied!');
        }
