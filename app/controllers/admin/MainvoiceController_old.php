...
class MainvoiceController extends AdminController {
    ...
    {
        $this->beforeFilter('hasAccess:manage_ma_invoices');
    ...
    public function getIndex()
    {
      ...
      //  $this->generate();
        ...
        $invoices = Mainvoice::orderBy('mainvoice.id', 'DESC')->paginate(20);
    ...
    public function getShow($id)
    {
        ...
        $merchantagreement = $invoice->merchantagreement();

      // foreach($merchantagreement as $mag) {echo $mag->paramval(2)->map_value;} exit;

        var_dump($merchantagreement->paramval(2)); exit;

        return View::make('backend/mainvoice/show', compact('invoice','specificationrows', 'costrows', 'reportrows', 'holdbackrows', 'customrows', 'heldrows', 'incomerows'));
    ...
    public function getPDF($invoiceId)
    {
        ...
        $view = View::make('backend/mainvoice/invoicepdf', compact('invoice','specificationrows', 'costrows', 'reportrows', 'holdbackrows', 'customrows', 'heldrows', 'incomerows'));
    ...
    public function generate()  /*@todo add generate($invoiceid)*/
    {
        ...
        //$merchants = Merchantagreement::get();
        ...
        foreach($merchantagreements as $ma)
        {
            ...
            $payout_date = $this->getPayoutDate($merchantagreementID);
            ...
            $amountevents = $this->getamountVE($merchantID, $date_from, $date_to);
                ...
                //Total specification
                $invrows['payable_sum'] = $payable_amount; // - $costrows['sum_costs']; //include costs????


            $conv_rate = $this->getConversionrate($merchantID, $date_to);
            ...
            $check = Mainvoice::where('merchant_id','=',$merchantagreementID)
             ...
             if($check)  //UPDATE EXISTING INVOICE
             {
                //Custom Costs Rows
                 ...
                 //Invoice Balance
                 $balance_in  = $this->getLastbalance($merchantID);

                 $balance_out = $balance_in + $held;

                 $inv = New Mainvoice;

                 $invoice = $inv->where('id', '=',$check->id )->update(
                    array('merchant_id'         => $merchantagreementID,
                          'date_from'           => $date_from,
                          'date_to'             => $date_to,
            ...
            else //CREATE NEW INVOICE
            {
                ...
                //Invoice Balance
                 ...
                 $balance_out = $balance_in; // No added held when invoice is just created
    ...
    public function lastinvoiceDate($merchantagreementID)
    {
        $lastinvoice = Mainvoice::join('mainvoicestatus','mainvoice.id','=','mainvoicestatus.mainvoice_id')
            ->where('merchant_id','=',$merchantagreementID)
    ...
    public function getPayoutDate($merchantagreementID)
    {
        ...
        $payoutDate = date('Y-m-d H:i:s', strtotime($dateTo. ' + '.$numdays.' days'));
    ...
    public function getTransactionRange($merchantID, $date_from, $date_to)
    {
      ...
      /*  $transactionid_from = Transaction::where('merchant_id','=',$merchantID)
    ...
    public function getConversionrate($day, $merchant)
    {
        /*@todo: Get conversion rate from voucher */
        $conversion_rate = 1;
    ...
    public function getLastbalance($merchant)
    {
        $lastbalance = Mainvoice::join('mainvoicestatus','mainvoice.id','=','mainvoicestatus.mainvoice_id')
            ->where('merchant_id','=',$merchant)
            ->where('mainvoicestatus.invoicestatus_id','=','3')  //status 3 paid
            ->where('mainvoice.date_to','<', $this->getDateFrom($merchant))
            ->orderBy('mainvoice.created_at','desc')
            ->first();
    ...
    public function getEdit($invoiceId = null)
    {
        ...
        return View::make('backend/mainvoice/edit', compact('invoice','specificationrows', 'costrows', 'reportrows', 'holdbackrows', 'customrows', 'heldrows', 'incomerows'));
    }
    ...
    public function postEdit($invoiceId = null)
    {
        ...
        //Update balance
        $invoice->balance_out = $invoice->balance_in + $allheld;
    ...
    public function getPay($invoiceId = null)
    {
        ...
        return View::make('backend/mainvoice/pay', compact('invoice','specificationrows', 'costrows', 'reportrows', 'holdbackrows', 'customrows', 'heldrows', 'incomerows'));
    }
