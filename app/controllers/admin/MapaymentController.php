...
class MapaymentController extends AdminController {
    /**
     * The invoice id
     * @var int
     */
    ...
    public function postPay($invoiceId = null)
    {
         ...
         * */
            ...
            /*============== Payment Secuencial ID */

            //  $secid_sql = "UPDATE mapayment SET paymentid = (mapayment_sec.row_number + 999) FROM (SELECT mapayment.id, mainvoice.merchantagreement_id, row_number() OVER (PARTITION BY merchantagreement_id ORDER BY mapayment.id) from mapayment join mainvoice on mapayment.mainvoice_id = mainvoice.id) AS mapayment_sec WHERE mainvoice.id = mainvoice_sec.id AND mainvoice.merchantagreement_id = '".$merchantagreementID."' ";
            $secid_sql = "UPDATE mapayment SET paymentid = (mapayment_sec.row_number + 999) FROM (SELECT mapayment.id, mainvoice.merchantagreement_id, row_number() OVER (PARTITION BY merchantagreement_id ORDER BY mapayment.id) from mapayment join mainvoice on mapayment.mainvoice_id = mainvoice.id) AS mapayment_sec WHERE mapayment.id = mapayment_sec.id AND mapayment_sec.merchantagreement_id = '".$invoice->merchantagreement_id."' ";
